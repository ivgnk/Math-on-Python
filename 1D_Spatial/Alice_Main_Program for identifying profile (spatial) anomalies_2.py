...
def calculate_convergence_criterion(zones_prev, zones_current):
    ...
    Если разница меньше 5%, считаем, что сходимость достигнута.
    """
    if len(zones_prev) == 0:
        return 1.0
...
def detect_anomaly_zones_iterative(data, x,
                  ...
                  max_iterations=20):
    """
    Интерактивный алгоритм поиска разбиения на 20–40 аномальных зон.

    Алгоритм:
    1. Начинаем с высокого порога Z‑score (initial_threshold), который даёт мало зон.
    2. На каждой итерации уменьшаем порог на фиксированную величину.
    3. Вычисляем аномальные зоны для текущего порога.
    4. Проверяем, попадает ли число зон в целевой диапазон (20–40).
    5. Если число зон слишком мало — продолжаем уменьшать порог.
    6. Если число зон попадает в диапазон — проверяем критерий сходимости.
    7. Если сходимость достигнута или превышено число итераций — останавливаемся.

    Параметры:
    - initial_threshold: начальный порог Z‑score
    - min_threshold: минимальный порог (нижняя граница поиска)
    - max_threshold: максимальный порог (верхняя граница поиска)
    - target_min_zones, target_max_zones: целевой диапазон числа зон
    - max_iterations: максимальное число итераций

    Возвращает: оптимальные зоны, финальный порог, историю итераций
    """
    ...
    for iteration in range(max_iterations):
        ...
        # Сортировка и ограничение числа зон
        zones.sort(key=lambda z: abs(z[2]), reverse=True)
        zones = zones[:target_max_zones * 2]  # временно оставляем больше для анализа

        # Критерий сходимости
        if iteration > 0:
            convergence = calculate_convergence_criterion(zones_history[-1], zones)
        else:
            convergence = 1.0
        ...
        in_target_range = target_min_zones <= len(zones) <= target_max_zones

        info_text = (
            f"Итерация: {iteration + 1}/{max_iterations}\n"
            f"Число аномальных зон: {len(zones)}\n"
        ...
        # Уменьшаем порог для следующей итерации
        threshold = max(threshold - step, min_threshold)

    # Выбираем лучшие зоны (наиболее близкие к целевому диапазону)
    best_idx = 0
    best_diff = float('inf')
    for i, zones in enumerate(zones_history):
        diff = abs(len(zones) - (target_min_zones + target_max_zones) / 2)
        if diff < best_diff:
            best_diff = diff
            best_idx = i

    return zones_history[best_idx], threshold, zones_history, convergence_history
