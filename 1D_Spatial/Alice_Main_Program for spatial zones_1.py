...
def main():
    ...
    if len(optimal_zones) == 0:
        ...
        for i, (start, end, mean_val) in enumerate(optimal_zones):
            # Исправленный вывод координат — используем end (уже скорректированный)
            print(f"Зона {i+1}:")
            print(f"  Координаты: {x[start]:.1f} – {x[end]:.1f}")
            print(f"  Длина: {end - start + 1} точек")  # +1 для корректного подсчёта
            print(f"  Среднее значение: {mean_val:.2f}")
            print("-"*40)

        # Проверка, что все точки покрыты
        total_covered = sum(zone[1] - zone[0] + 1 for zone in optimal_zones)  # +1 для каждой зоны
        print(f"\nПРОВЕРКА ПОКРЫТИЯ:")
