"""
Alice
Программа выделения профильных (пространственных) аномалий
Program for identifying profile (spatial) anomalies

Цель: объединить соседние аномальные точки в единые зоны.

Алгоритм:
А) Найти все непрерывные сегменты, где anomaly_mask == True.
Б) Отфильтровать слишком короткие сегменты (например, <3 точек).
В) Объединить близкие аномальные сегменты (если расстояние между ними <5 точек).
"""
